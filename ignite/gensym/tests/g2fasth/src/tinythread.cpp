...
bool thread::join(const chrono::milliseconds& timeout) //  timeout in ms
{
  if (!joinable())
    return true;
  ...
  bool in_time;
#if defined(_TTHREAD_WIN32_)
  in_time = WAIT_OBJECT_0 == WaitForSingleObject(native_handle(), (DWORD)ms);
  if (in_time)
    CloseHandle(mHandle);
#elif defined(_TTHREAD_POSIX_)
  timespec ts;
  clock_gettime(CLOCK_REALTIME, &ts);                       // get current (absolute) time
  ts.tv_sec  += ms / 1000;                                  // adding seconds
  ts.tv_nsec += (ms % 1000) * 1000000;                      // adding nanoseconds
